...
import {block} from './characters.js';
export function createWorld(scene){const solids=[],labels=[],trees=[],clouds=[];const add=(mesh,solid=false)=>{scene.add(mesh);if(solid)solids.push(new THREE.Box3().setFromObject(mesh));return mesh;};const cube=(w,h,d,c,x,y,z,solid=false)=>add(block(w,h,d,c,x,y,z),solid);
...
function building(x,z,w,d,color,roof,title){cube(w+.8,.25,d+.8,0xe8e1cc,x,.1,z);cube(w,3.5,d,color,x,1.9,z,true);cube(w+.8,.5,d+1,roof,x,3.85,z);cube(w+.2,.4,d+.5,roof,x,4.3,z);cube(w-1,.35,d-.4,roof,x,4.65,z);cube(1.3,2.4,.1,0x6a503b,x,1.35,z+d/2+.06);for(const side of [-1,1]){cube(1.7,1.4,.1,0x9cd9e9,x+side*w*.31,2.05,z+d/2+.08);cube(.08,1.4,.12,0xfff6de,x+side*w*.31,2.05,z+d/2+.14);cube(1.7,.08,.12,0xfff6de,x+side*w*.31,2.05,z+d/2+.14);}label(title,x,5.5,z);}
building(-24,-3,7,6,0xffe7a5,0xc97550,'RUMAH PEMAIN');building(0,-4,10,7,0xfaf2ce,0x368c7c,'DEWAN KOMUNITI');building(24,-3,9,6,0xf6ca92,0xef7c68,'PASAR RAYA');building(0,18,8,6,0xf6ddad,0xb77790,'RUMAH TERBUKA');building(0,40,10,6,0xc4e5dc,0x4685ac,'PUSAT INTERNET DESA');building(-24,18,8,6,0xf9e8bf,0x688cb1,'BALAI KOMUNITI');
...
for(const x of [-5,5]){bench(x,-24);bench(x,-30);}label('TAMAN REKREASI',0,3,-29);cube(10,.09,10,0x94c983,0,0,-28);
...
return {solids,labels,update(t,dt){trees.forEach((v,i)=>v.rotation.z=Math.sin(t*.8+i)*.012);clouds.forEach(c=>{c.position.x+=dt*.16;if(c.position.x>47)c.position.x=-47;});}};
}
